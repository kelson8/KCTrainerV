...
Ped CreatePoolClonePed(Ped pedToClone)
{
	...
	SET_ENTITY_COORDS(clone, pos, false, false, false, false);
...
{
	LoadModel(modelHash);
	Ped ped = CREATE_PED(pedType, modelHash, Vector3(x, y, z), heading, true, false);
	HandleEntity(ped);
...
Ped CreateRandomPoolPed(float x, float y, float z, float heading)
{
	...
	{
		ped = CREATE_RANDOM_PED(Vector3(x, y, z));
	...
	for (int i = 0; i < 12; i++)
	{
		...
		if (i < 4)
		{
			...
			SET_PED_PROP_INDEX(ped, i, propDrawable, propTexture, true, 1);
		}
	}
...
{
	LoadModel(modelHash);
	auto veh = CREATE_VEHICLE(modelHash, Vector3(x, y, z), heading, true, false, false);
	HandleEntity(veh);
	SET_MODEL_AS_NO_LONGER_NEEDED(modelHash);
	return veh;
}
...
Vehicle CreatePoolCloneVehicle(Vehicle vehToClone)
{
	...
	SET_ENTITY_VELOCITY(clone, Vector3(velocity.x, velocity.y, velocity.z));
...
{
	LoadModel(modelHash);
	auto prop = CREATE_OBJECT(modelHash, Vector3(x, y, z), true, false, dynamic);
	HandleEntity(prop);
	SET_MODEL_AS_NO_LONGER_NEEDED(modelHash);
	return prop;
}
                                   ...
                                   float rotX, float rotY, float rotZ, bool softPinning, bool collision, bool fixedRot)
{
	...
	ATTACH_ENTITY_TO_ENTITY(prop, ped, boneIndex, Vector3(offsetX, offsetY, offsetZ), Vector3(rotX, rotY, rotZ), false, softPinning,
