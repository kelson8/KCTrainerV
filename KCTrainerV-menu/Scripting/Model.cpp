...
namespace GTAmodel
{
	...
	//ModelDimensions::ModelDimensions(const Vector3M& d1, const Vector3M& d2) : Dim1(d1), Dim2(d2)
	...
	// TODO Fix this
	//void Model::Dimensions(Vector3M_t& dim1, Vector3M_t& dim2) const
	void Model::Dimensions(Vector3& dim1, Vector3& dim2) const
	{
		/*GET_MODEL_DIMENSIONS(this->hash, &dim1, &dim2);
		dim1.x = abs(dim1.x); dim2.x = abs(dim2.x);
		dim1.y = abs(dim1.y); dim2.y = abs(dim2.y);
