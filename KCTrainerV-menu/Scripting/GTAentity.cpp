...
void GTAentity::Dynamic_set(bool value)
{
	...
	if (value)
	{
		...
		SET_ENTITY_VELOCITY(this->mHandle, Vector3(0.0f, 0.0f, 0.0f));
	}
}
...
float GTAentity::GetGroundZ() const
{
	...
	GET_GROUND_Z_FOR_3D_COORD(Vector3(pos.x, pos.y, 1100.0f), &pos.z, 0, 0);
	return pos.z;
}
...
void GTAentity::PlaceOnGround()
{
	...
	SET_ENTITY_COORDS(this->mHandle, pos, 0, 0, 0, 1);
...
void GTAentity::Position_set(Vector3 value)
{
	...
	SET_ENTITY_COORDS_NO_OFFSET(this->mHandle, value, 1, 1, 1);
}
...
void GTAentity::Velocity_set(Vector3 value)
{
	...
	SET_ENTITY_VELOCITY(this->mHandle, value);
}
...
bool GTAentity::IsInArea(Vector3 pos1, Vector3 pos2, float angle) const
{
	...
	return IS_ENTITY_IN_ANGLED_AREA(this->mHandle, pos1, pos2, angle, true, true, true) != 0;
}
bool GTAentity::IsNearEntity(GTAentity entity, Vector3 distance) const
{
	...
	return IS_ENTITY_AT_ENTITY(this->mHandle, entity.Handle(), distance, 0, 1, 0) != 0;
}
...
Vector3 GTAentity::GetOffsetInWorldCoords(Vector3 offset) const
{
	...
	return GET_OFFSET_FROM_ENTITY_IN_WORLD_COORDS(this->mHandle, offset);
}
Vector3 GTAentity::GetOffsetInWorldCoords(float X, float Y, float Z) const
{
	...
	return GET_OFFSET_FROM_ENTITY_IN_WORLD_COORDS(this->mHandle, Vector3(X, Y, Z));
}
...
void GTAentity::AttachTo(GTAentity entity, int boneIndex, bool collision, Vector3 position, Vector3 rotation)
{
	...
	ATTACH_ENTITY_TO_ENTITY(this->mHandle, entity.Handle(), boneIndex, position, rotation, false, false, collision, false, 2, true, 0);
}
void GTAentity::AttachTo(GTAentity entity, int boneIndex, Vector3 position, Vector3 rotation, bool b9, bool useSoftPinning, bool collision, bool isPed, int vertexIndex, bool fixedRot)
{
	...
	ATTACH_ENTITY_TO_ENTITY(this->mHandle, entity.Handle(), boneIndex, position, rotation, b9, useSoftPinning, collision, isPed, vertexIndex, fixedRot, 0);
}
...
void GTAentity::AttachPhysicallyTo(GTAentity entity, int boneIndexDoer, int boneIndexGetter, float forceToBreak, Vector3 position1, Vector3 position2, Vector3 rotation)
{
	...
	ATTACH_ENTITY_TO_ENTITY_PHYSICALLY(this->mHandle, entity.Handle(), boneIndexDoer, boneIndexGetter, position1, position2, rotation, forceToBreak, 1, 1, 1, 1, 2);
}
...
void GTAentity::ApplyForce(Vector3 direction, Vector3 offset, ForceType forceType)
{
	...
	APPLY_FORCE_TO_ENTITY(this->mHandle, static_cast<int>(forceType), direction, offset, false, false, true, true, false, true);
}
...
void GTAentity::ApplyForceRelative(Vector3 direction, Vector3 offset, ForceType forceType)
{
	...
	APPLY_FORCE_TO_ENTITY(this->mHandle, static_cast<int>(forceType), direction, offset, false, true, true, true, false, true);
}
void GTAentity::ApplyForceCustom(Vector3 direction, Vector3 offset, ForceType forceType, bool unk1, bool isRel, bool ignoreUpVector, bool unk2, bool unk3, bool unk4)
{
	...
	APPLY_FORCE_TO_ENTITY(this->mHandle, static_cast<int>(forceType), direction, offset, unk1, isRel, ignoreUpVector, unk2, unk3, unk4);
}
...
void GTAentity::Delete(bool tele)
{
	...
	if (tele) SET_ENTITY_COORDS_NO_OFFSET(this->mHandle, Vector3(32.2653f, 7683.5249f, 0.5696f), 0, 0, 0);
