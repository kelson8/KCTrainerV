...
// Well, with a little modifications, this file seems to have worked.
...
namespace Game
{
	...
	namespace Sound
	{
		...
		// TODO Test this
		void GameSound::Play(Entity entity)
		{
			soundID = GET_SOUND_ID();
			//PLAY_SOUND_FROM_ENTITY(soundID, (PCHAR)sound.c_str(), entity.Handle(), (PCHAR)soundSet.c_str(), 0, 0);
			PLAY_SOUND_FROM_ENTITY(soundID, (PCHAR)sound.c_str(), entity, (PCHAR)soundSet.c_str(), 0, 0);
			active = true;
		}
	...
	// TODO Reuse these for my KCTrainerV, might be useful.
	namespace Print
	{
		// Game - Print/draw
		void setupdraw()
		...
		void drawstring(const std::string& s, float X, float Y)
		{
			...
			END_TEXT_COMMAND_DISPLAY_TEXT(Vector2(X, Y), 0);
		}
	...
	std::string InputBox(const std::string& escReturn, int maxChars, std::string titlegxt, std::string preText)
	{
		...
		while (UPDATE_ONSCREEN_KEYBOARD() == 0)
		{
			...
			END_TEXT_COMMAND_DISPLAY_TEXT(Vector2(0.5f, 0.37f), 0);
			//WAIT(0);
		}
