...
void GTAvehicle::Delete(bool tele)
{
	...
	if (tele)
		...
		SET_ENTITY_COORDS_NO_OFFSET(this->mHandle, Vector3(32.2653f, 7683.5249f, 0.5696f), 0, 0, 0);
...
void GTAvehicle::ApplyDamage(Vector3 loc, float damageAmount, float radius)
{
	...
	SET_VEHICLE_DAMAGE(this->mHandle, loc, damageAmount, radius, true);
}
...
GTAentity GTAvehicle::CreateRandomPedOnSeat(VehicleSeat seat)
{
	...
	else
	{
		...
		GTAentity ped = CREATE_RANDOM_PED(Vector3(0.0f, 0.0f, 0.0f));
