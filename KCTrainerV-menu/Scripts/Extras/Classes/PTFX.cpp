...
#pragma region LoopedPTFX
	...
	void LoopedPTFX::Start(GTAentity entity, float scale, const Vector3& offset, const Vector3& rotation, RGBA col, int bone)
	{
		...
		if (bone == -1)
		{
			...
			this->mHandle = START_PARTICLE_FX_LOOPED_ON_ENTITY((PCHAR)this->effect.c_str(), entity.Handle(), offset, rotation, scale, 0, 0, 0);
		}
		else
		{
			if (entity.IsPed())
			...
			this->mHandle = START_PARTICLE_FX_LOOPED_ON_ENTITY_BONE((PCHAR)this->effect.c_str(), entity.Handle(), offset, rotation, bone, scale, 0, 0, 0);
		}
	...
	void LoopedPTFX::Start(const Vector3& position, float scale, const Vector3& rotation)
	{
		...
		this->mHandle = START_PARTICLE_FX_LOOPED_AT_COORD((PCHAR)this->effect.c_str(),
			...
			position, rotation, scale, false, false, false, false);
	}
	...
	void LoopedPTFX::SetOffsets(const Vector3& offset, const Vector3& rotOffset)
	{
		...
		SET_PARTICLE_FX_LOOPED_OFFSETS(this->mHandle, offset, rotOffset);
	}
	...
	void LoopedPTFX::RemoveInRange(const Vector3& position, float radius)
	{
		...
		REMOVE_PARTICLE_FX_IN_RANGE(position, radius);
	}
#pragma endregion
...
#pragma region NonLoopedPTFX
	...
	void NonLoopedPTFX::Start(GTAentity entity, float scale, const Vector3& offset, const Vector3& rotation, RGBA col, int pedBone)
	{
		...
		if (pedBone == -1)
		{
			...
			START_NETWORKED_PARTICLE_FX_NON_LOOPED_ON_ENTITY((PCHAR)this->effect.c_str(), entity.Handle(), offset, rotation, scale, false, false, false);
		}
		else
		{
			START_NETWORKED_PARTICLE_FX_NON_LOOPED_ON_PED_BONE((PCHAR)this->effect.c_str(), entity.Handle(), offset, rotation, pedBone, scale, false, false, false);
		}
	}
	// pedBone does not require GetBoneIndex for peds.
	...
	void trigger_ptfx_1(const std::string& asset, const std::string& name, GTAentity entity, Vector3 position, Vector3 rotation, float scale, int pedBone)
	{
		...
		if (HAS_NAMED_PTFX_ASSET_LOADED(asset.c_str()))
		{
			...
			if (entity.Handle() == 0)
				...
				START_NETWORKED_PARTICLE_FX_NON_LOOPED_AT_COORD(name.c_str(), position, rotation, scale, false, false, false, false);
			else
			{
				if (entity.Exists())
				{
					...
					if (pedBone != -1)
					{
						...
						START_NETWORKED_PARTICLE_FX_NON_LOOPED_ON_PED_BONE(name.c_str(), entity.Handle(), position
							, rotation, pedBone, scale, false, false, false);
					}
					else
					{
						...
						START_NETWORKED_PARTICLE_FX_NON_LOOPED_ON_ENTITY(name.c_str(), entity.Handle(), position, 
							rotation, scale, false, false, false);
					}
				}
			}
		}
