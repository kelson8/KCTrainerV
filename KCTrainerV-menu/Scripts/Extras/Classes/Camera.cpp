...
// TODO Fix this file, required for some other files and to get World.cpp working.
...
void Camera::Heading_set(float value)
{
	...
	SET_CAM_ROT(this->mHandle, Vector3(oldRot.x, oldRot.y, value), 2);
}
...
void Camera::AttachTo(GTAentity entity, Vector3 offset)
{
	...
	ATTACH_CAM_TO_ENTITY(this->mHandle, entity.Handle(), offset, true);
}
void Camera::AttachTo(GTAped ped, int boneIndex, Vector3 offset)
{
	...
	ATTACH_CAM_TO_PED_BONE(this->mHandle, ped.Handle(), boneIndex, offset, true);
}
void Camera::Detach()
{
...
void Camera::PointAt(Vector3 target)
{
	...
	POINT_CAM_AT_COORD(this->mHandle, target);
}
...
void Camera::PointAt(GTAped target, int boneIndex)
{
	...
	POINT_CAM_AT_PED_BONE(this->mHandle, target.Handle(), boneIndex, Vector3(0.0f, 0.0f, 0.0f), true);
}
void Camera::PointAt(GTAped target, int boneIndex, Vector3 offset)
{
	...
	POINT_CAM_AT_PED_BONE(this->mHandle, target.Handle(), boneIndex, offset, true);
}
