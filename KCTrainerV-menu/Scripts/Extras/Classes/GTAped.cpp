...
void GTAped::Delete(bool tele)
{
	...
	if (tele) SET_ENTITY_COORDS_NO_OFFSET(this->mHandle, Vector3(32.2653f, 7683.5249f, 0.5696f), false, false, false);
...
Vector3 GTAped::GetBoneCoord(int boneID) const
{
	...
	return GET_PED_BONE_COORDS(this->mHandle, boneID, Vector3(0, 0, 0));
}
Vector3 GTAped::GetBoneCoord(int boneID, const Vector3& offset) const
{
	...
	return GET_PED_BONE_COORDS(this->mHandle, boneID, Vector3(offset.x, offset.y, offset.z));
}
...
void GTAped::ApplyBlood(const std::string& bloodDecalName, int boneID, const Vector3& rotation)
{
	...
	APPLY_PED_BLOOD(this->mHandle, boneID, rotation, (PCHAR)bloodDecalName.c_str());
}
...
void GTAped::ApplyDamageDecal(const std::string& damageDecalName, int boneID, const Vector3& rotation, float damageScale, float multiplier, int state, bool b1)
{
	...
	APPLY_PED_DAMAGE_DECAL(this->mHandle, boneID, rotation, damageScale, multiplier, state, b1, (PCHAR)damageDecalName.c_str());
}
#endif
