...
__int64 HK_rage__scrThread__Run(rage::scrThread *thread)
{
	...
	if (!strcmp(thread->GetName(), "shop_controller"))
	{
		...
		{
			std::string logMessage = std::format("New shop_controller script instance with thread ID {}!", threadId);
			LOG(INFO, logMessage);
			//LOG(INFO, "New shop_controller script instance with thread ID " << threadId << "!");
			...
			if (program->m_CodeBlocks)
			{
				...
				if (!handle.IsValid())
				{
					LOG(ERROR, "Error while bypassing online vehicle despawn mechanism; spawned online vehicles will "
				...
				else
				{
					...
					LOG(INFO, "Online vehicle despawn mechanism successfully bypassed");
				}
			}
		}
	}
