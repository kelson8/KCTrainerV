...
CVehicleModelInfo* initVehicleArchetype_stub(const char* name, bool a2, unsigned int a3) {
	g_vehicleHashes.insert({ MISC::GET_HASH_KEY(name), boost::to_lower_copy(name) });
...
void setupHooks() {
	...
	std::string logMsg = std::format("Found InitVehicleArchetype at ", std::to_string(addr));
	LOG(INFO, logMsg);
...
if (Vector3M::Subtract(_position, Vector3M(position[0], position[1], position[2])).LengthSquared() > _radiusSquared)
{
return false;
}
}
...
void GTAmemory::Init()
{
	...
	if (getGameVersion() <= 57) {
		...
		if (!address) {
			std::string errorLogMessage = "Couldn't find GetModelInfo";
		...
		if (!address) {
			std::string errorLogMessage = "Couldn't find GetModelInfo (v58+)";
			LOG(INFO, errorLogMessage);
...
Vector3M GTAmemory::ReadVector3M(UINT64 address)
{
	const float* data = (float*)address;
	return Vector3M(data[0], data[1], data[2]);
}
void GTAmemory::WriteVector3M(UINT64 address, const Vector3M& value)
{
...
void GTAmemory::GetVehicleHandles(std::vector<Entity>& result, const Vector3M& position, float radius)
{
	auto pool = EntityPoolTask(EntityPoolTask::Type::Vehicle);
...
void GTAmemory::GetVehicleHandles(std::vector<Entity>& result, const Vector3M& position, float radius, std::vector<DWORD> modelHashes)
{
	auto pool = EntityPoolTask(EntityPoolTask::Type::Vehicle);
	pool._position = position;
	pool._radiusSquared = radius * radius;
...
void GTAmemory::GetPedHandles(std::vector<Entity>& result, const Vector3M& position, float radius)
{
	auto pool = EntityPoolTask(EntityPoolTask::Type::Ped);
	pool._position = position;
	pool._radiusSquared = radius * radius;
...
void GTAmemory::GetPedHandles(std::vector<Entity>& result, const Vector3M& position, float radius, std::vector<DWORD> modelHashes)
{
	auto pool = EntityPoolTask(EntityPoolTask::Type::Ped);
	pool._position = position;
	pool._radiusSquared = radius * radius;
...
void GTAmemory::GetPropHandles(std::vector<Entity>& result, const Vector3M& position, float radius)
{
	auto pool = EntityPoolTask(EntityPoolTask::Type::Prop);
	pool._position = position;
	pool._radiusSquared = radius * radius;
...
void GTAmemory::GetPropHandles(std::vector<Entity>& result, const Vector3M& position, float radius, std::vector<DWORD> modelHashes)
{
	auto pool = EntityPoolTask(EntityPoolTask::Type::Prop);
	pool._position = position;
	pool._radiusSquared = radius * radius;
...
void GTAmemory::GetEntityHandles(std::vector<Entity>& result, const Vector3M& position, float radius)
{
	auto pool = EntityPoolTask(EntityPoolTask::Type::Ped | EntityPoolTask::Type::Vehicle | EntityPoolTask::Type::Prop);
	pool._position = position;
	pool._radiusSquared = radius * radius;
...
void GTAmemory::GetEntityHandles(std::vector<Entity>& result, const Vector3M& position, float radius, std::vector<DWORD> modelHashes)
{
	auto pool = EntityPoolTask(EntityPoolTask::Type::Ped | EntityPoolTask::Type::Vehicle | EntityPoolTask::Type::Prop);
	pool._position = position;
	pool._radiusSquared = radius * radius;
...
void GTAmemory::GetPickupObjectHandles(std::vector<int>& result, const Vector3M& position, float radius)
{
	auto pool = EntityPoolTask(EntityPoolTask::Type::PickupObject);
	pool._position = position;
	pool._radiusSquared = radius * radius;
...
void SpSnow::EnableSnow(bool bEnable)
{
	...
	if (bEnable)
	{
		...
		if (isMinGameVersion3095)
		{
			...
			// TODO Figure out this native, it doesn't seem to exist in my natives list.
...
{	
	...
	for (int i = 0; i < shopController->CodePageCount(); i++)
	{
		...
		if (size)
		{
			...
			if (address)
			{
				...
				std::string logMessage = std::format("Setting Global Variable {} to true", std::to_string(globalindex));
				LOG(INFO, logMessage);
	...
	std::string errorLogMessage = "Global Variable not found, check game version >= 1.0.678.1";
	LOG(INFO, errorLogMessage);
...
bool GTAmemory::FindShopController() {
	...
	if (!patternAddr) {
		std::string errorLogMessage = "ERROR: finding address 1, aborting...";
	...
	if (Item == NULL) {
		std::string errorLogMessage = "ERROR: finding script 0x39DA738B, aborting...";
		LOG(ERROR, errorLogMessage);
