...
namespace Memory
{
	inline std::vector<Hash> GetAllVehModels()
	{
		...
		if (vehModels.empty())
		{
			...
			static const std::unordered_set<Hash> blacklistedModels {
				MISC::GET_HASH_KEY("arbitergt"), MISC::GET_HASH_KEY("astron2"), MISC::GET_HASH_KEY("cyclone2"), MISC::GET_HASH_KEY("ignus2")
				, MISC::GET_HASH_KEY("s95"),
			};
	...
	inline void SetVehicleOutOfControl(Vehicle vehicle, bool state)
	{
		...
		if (vehClass == 15 || vehClass == 16
		    || vehModel == MISC::GET_HASH_KEY("BLIMP")) // No helis or planes, also make sure to explicitely exclude blimps at all
		...
		static auto outOfControlStateOffset = []() -> WORD
		{
			...
			if (!handle.IsValid())
			{
				LOG(WARN, "Vehicle out of control state offset not found!");
				return 0;
			}
	...
	inline bool IsVehicleBraking(Vehicle vehicle)
	{
		static auto brakeStateOffset = []() -> WORD
		{
			...
			if (!handle.IsValid())
			{
				LOG(WARN, "Vehicle brake state offset not found!");
				return 0;
			}
	...
	// This seems to have an error, I just disabled it for now.
